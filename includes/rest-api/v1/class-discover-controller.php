	...
	private function describe_banner(): array {
		return [
			...
			'fields' => [
				...
				'device'       => [ 'type' => 'enum', 'values' => [ 'desktop', 'mobile' ], 'required' => true ],
				'image_id'     => [ 'type' => 'integer', 'required' => true, 'note' => 'WP attachment ID. Upload via the Media Library REST API first. On PUT, swaps the banner image in place.' ],
				'image'        => [ 'type' => 'object', 'readonly' => true, 'shape' => '{ id, url, width, height, srcset, sizes, alt }' ],
				'name'         => [ 'type' => 'string|null', 'note' => 'Internal admin-facing label ("Black Friday hero"). Separate from alt_text. Empty string clears it.' ],
				...
				'sort_order'   => [ 'type' => 'integer', 'note' => 'Order within the group. Auto-assigned to next position on append; PUT /banners/{bid} {sort_order:N} or POST /groups/{gid}/banners/reorder to move.' ],
	...
	private function describe_endpoints( string $base ): array {
		return [
			...
			[
				...
				'path'     => '/banners/{bid}',
				'auth'     => 'write',
				'summary'  => 'Partial update of a single banner — toggle is_active, change name, image_id, link, target, alt, sort_order, group.',
			],
			[
				'method'   => 'DELETE',
				'path'     => '/banners/{bid}',
			...
			[
				'method'   => 'POST',
				'path'     => '/banners/{bid}/duplicate',
				'auth'     => 'write',
				'summary'  => 'Clone a banner into the same group. " (copy)" is appended to the name if set.',
				'returns'  => '{ duplicated: true, id: number, source_id: number }',
			],
			[
				'method'   => 'POST',
				'path'     => '/groups/{gid}/banners/reorder',
				'auth'     => 'write',
				'summary'  => 'Rewrite sort_order for all banners in a group in one round-trip. Body: { order: [bid, bid, ...] }. Scoped by group_id so ids that don\'t belong to the group are ignored.',
				'returns'  => 'campaign',
			],
